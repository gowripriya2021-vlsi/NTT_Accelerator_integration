...
int cypressflashIdentification(){
    ...
    if(value==CYPRESS_MEM_ID)
        printf("\tCypress: S25FS512S Detected value: %08x \n", value);
    else{
...
int cypressEnterXIP(int address, int data_length){
    ...
    set_qspi_shakti32(dlr,data_length); //DLR
    set_qspi_shakti32(ccr,(CCR_DDRM | CCR_FMODE(CCR_FMODE_INDRD)|CCR_DMODE(QUAD)|CCR_DUMMY_CONFIRMATION|CCR_DCYC(9)|CCR_ADSIZE(FOURBYTE)|CCR_ADMODE(QUAD)|CCR_IMODE(SINGLE)|CCR_INSTRUCTION(0xEC)));
    set_qspi_shakti32(ar,address); //Address where the Config_string is situated in the micron nand flash memory 
    waitfor(1000);
    wait_for_tcf(status);
    printf("Status : %08x dcr : %08x cr : %08x ccr : %08x dlr: %08x ar: %08x\n",status,*dcr,*cr,*ccr,*dlr,*ar);
    ...
    printf("\t Read Data is %08x\n",read_data);
    reset_interrupt_flags();
    set_qspi_shakti32(ccr, (CCR_DDRM | CCR_FMODE(CCR_FMODE_MMAPD)|CCR_DMODE(QUAD)|CCR_DUMMY_CONFIRMATION|CCR_DCYC(9)|CCR_ADSIZE(FOURBYTE)|CCR_ADMODE(QUAD)|CCR_IMODE(NDATA))); //Entering MMAPD
    return 0;
}
