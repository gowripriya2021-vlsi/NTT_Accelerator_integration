...
def random_read():
    ...
    for i in range(8387000):
#maxaddr=262143
        address = int(random.randrange(8, 262143, 4))
        access_size = random.choice([byte,hword,word, dword])
        sign = random.choice([signed,unsigned])
        access = random.choice([read,write])
        insertdelay = random.choice([delay,nodelay])
        write_to_file(address, access, access_size, sign, insertdelay, nofence)
        entrycount = entrycount + 1
...
if entrycount > 8388608:
    print('Exceeded max count')
    sys.exit(1)
while entrycount<8388608:
    write_to_file(0,endsim,byte,signed,nodelay,nofence)
    entrycount=entrycount+1
