...
`endif 
    ...
    */
    ...
    Reg#(TLB_core_request#(vaddr, asidwidth)) rg_miss_req <- mkReg(?);
    ...
    rule rl_response if(!rg_tlb_miss[0]); 
        ...
        `endif
            ...
            ff_response_to_core.enq(ITLB_core_response{ address  : truncate(coreresp),
                                                      ...
                                                      `ifdef hypervisor   
                                                        ,gpa    : ? // as this fault not guest page fault
                                                      `endif });
        ...
        /* `ifndef sv32
          ...
          if (req.v == 1)begin 
            // check page fault           
            page_fault2 = fn_fault_check (resp.gpa_permissions, req.priv, req.v, True );
          end        
          page_fault = fn_fault_check (resp.hpa_permissions, req.priv, req.v, False );                                
    ...
    rule rl_response_frm_ptw if (rg_tlb_miss[1]);
        ...
        /*doc:note: in itlb the ptwalk response is also responsible to send the response
                    ...
                    of vpn*/
        //hpa creation
        // let m1 = Gen_add{addr   : rg_miss_req.address,
        //                 ppn    : resp.hpa, 
        //                 levels : resp.levels_G,
        //                 n      : unpack(resp.n)};
			  // let hpa = fn_gen_phy_address(m1);  
        //ppn creation 
        let m2 = Gen_add{addr   : rg_miss_req.address,
                        ppn    : resp.ppn, 
                        levels : resp.levels,
                        n      : unpack(resp.n)};
			  let ppn = fn_gen_phy_address(m2);  

        //gpa creation 
        // let m3 = Gen_add{addr   : rg_miss_req.address,
        //                 ppn    : resp.gpa, 
        //                 levels : resp.levels,
        //                 n      : unpack(resp.n)};
			  // let gpa = fn_gen_phy_address(m3); 

        
		    ff_response_to_core.enq(ITLB_core_response{ address : truncate(ppn),
                                                    trap    : resp.trap,
                                                    cause   : resp.cause
                                                  `ifdef hypervisor   
                                                    // ,gpa    : gpa
                                                  `endif });
        `logLevel( itlb, 0, $format("[%2d]ITLB : Sending (rl_ptw) PA:%h Trap:%b",hartid, ppn, resp.trap))
    endrule
